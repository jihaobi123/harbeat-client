...
document.addEventListener('keydown',e=>{if(!track||['TEXTAREA','SELECT'].includes(e.target.tagName))return;if(e.code==='Space'){e.preventDefault();play(selected);return}const s=track.segments[selected];if(!segmentEditable(s))return;if(e.key.toLowerCase()==='a')save(selected,{human_label:targetLabel(s.structure_label_candidate),uncertain:false,boundary_ok:true});else if(e.key.toLowerCase()==='u')save(selected,{human_label:'',human_confidence:'',uncertain:true,boundary_ok:true});else if(e.key.toLowerCase()==='b')save(selected,{human_label:'',human_confidence:'',uncertain:false,boundary_ok:false});else if(/^[1-8]$/.test(e.key))save(selected,{human_label:LABELS[+e.key-1],uncertain:false,boundary_ok:true})});
