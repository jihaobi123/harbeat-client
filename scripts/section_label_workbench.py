...
function renderContent(){if(!track){document.querySelector('#content').innerHTML='<p>请选择歌曲。</p>';return}const disabled=data.access.read_only?'disabled':'';const cards=track.segments.map((s,i)=>{const a=s.annotation||{},human=a.human_label||'';const buttons=LABELS.map((l,j)=>`<button ${disabled} class="label ${l===s.structure_label_candidate?'current':''} ${l===human?'human':''}" data-i="${i}" data-label="${l}">${j+1}.${ZH[l]}</button>`).join('');return `<article class="segment ${i===selected?'selected':''}" data-seg="${i}"><div class="row"><b>段 ${i+1}</b><button class="label play" data-i="${i}">▶ ${s.start.toFixed(2)}–${s.end.toFixed(2)}s</button><span>SongFormer：<b>${ZH[s.structure_label_candidate]||s.structure_label_candidate}</b></span>${human?`<span class="done">人工：<b>${ZH[human]||human}</b></span>`:''}${a.boundary_ok===false?'<span class="warning">边界有问题</span>':''}${a.uncertain?'<span class="warning">不确定</span>':''}</div><p class="prob muted">${esc(probs(s))}</p><div class="row">${buttons}<button ${disabled} class="label accept" data-i="${i}">A.接受原标签</button><button ${disabled} class="label uncertain" data-i="${i}">U.不确定</button><button ${disabled} class="label boundary" data-i="${i}">B.边界问题</button><select ${disabled} class="confidence" data-i="${i}"><option value="high" ${a.human_confidence==='high'?'selected':''}>高信心</option><option value="medium" ${a.human_confidence==='medium'?'selected':''}>中信心</option><option value="low" ${a.human_confidence==='low'?'selected':''}>低信心</option></select></div><textarea ${disabled} data-note="${i}" placeholder="可选备注">${esc(a.notes||'')}</textarea></article>`}).join('');document.querySelector('#content').innerHTML=`<h2>${esc(track.display_name)}</h2><p class="muted">${esc(track.style)} · ${track.split==='test'?'锁定测试集':'开发集'} · ${data.access.read_only?'只读检查':'负责分片，可编辑'}</p><audio id="audio" controls preload="metadata" src="/audio/${encodeURIComponent(track.track_id)}"></audio>${cards}`;bind();scrollSelected(false)}
function bind(){document.querySelectorAll('[data-seg]').forEach(el=>el.onclick=e=>{if(!e.target.closest('button,textarea,select')){selected=+el.dataset.seg;renderContent()}});document.querySelectorAll('.play').forEach(b=>b.onclick=()=>play(+b.dataset.i));if(data.access.read_only)return;document.querySelectorAll('[data-label]').forEach(b=>b.onclick=()=>save(+b.dataset.i,{human_label:b.dataset.label,uncertain:false,boundary_ok:true}));document.querySelectorAll('.accept').forEach(b=>b.onclick=()=>{const s=track.segments[+b.dataset.i];save(+b.dataset.i,{human_label:s.structure_label_candidate,uncertain:false,boundary_ok:true})});document.querySelectorAll('.uncertain').forEach(b=>b.onclick=()=>save(+b.dataset.i,{human_label:'',human_confidence:'',uncertain:true,boundary_ok:true}));document.querySelectorAll('.boundary').forEach(b=>b.onclick=()=>save(+b.dataset.i,{human_label:'',human_confidence:'',uncertain:false,boundary_ok:false}));document.querySelectorAll('[data-note]').forEach(t=>t.onchange=()=>save(+t.dataset.note,{notes:t.value},false));document.querySelectorAll('.confidence').forEach(s=>s.onchange=()=>save(+s.dataset.i,{human_confidence:s.value},false))}
...
async function save(i,patch,advance=true){if(data.access.read_only)return;selected=i;const confidence=document.querySelector(`.confidence[data-i="${i}"]`)?.value||'high';if(!Object.prototype.hasOwnProperty.call(patch,'human_confidence'))patch.human_confidence=confidence;const r=await fetch('/api/annotation',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({access_key:accessKey,track_id:track.track_id,segment_index:i,patch})});if(!r.ok){alert((await r.json()).error||'保存失败');await loadData(true);return}if(advance&&i+1<track.segments.length)selected=i+1;await loadData(true)}
async function loadData(preserve=true){if(loading)return;loading=true;const current=preserve&&track?track.track_id:null;try{const r=await fetch(`/api/dataset?key=${encodeURIComponent(accessKey)}`,{cache:'no-store'});if(!r.ok)throw new Error((await r.json()).error||'访问链接无效');data=await r.json();track=current?data.tracks.find(t=>t.track_id===current)||null:null;if(!track&&data.tracks.length)track=data.tracks[0];renderTracks();renderContent()}catch(e){document.querySelector('#content').innerHTML=`<h2 class="warning">无法打开标注数据</h2><p>${esc(e.message)}</p><p class="muted">请使用服务启动时生成的 Part 1、Part 2 或只读评估链接。</p>`}finally{loading=false}}
...
document.addEventListener('keydown',e=>{if(!track||['TEXTAREA','SELECT'].includes(e.target.tagName))return;if(e.code==='Space'){e.preventDefault();play(selected);return}if(data.access.read_only)return;if(e.key.toLowerCase()==='a'){const s=track.segments[selected];save(selected,{human_label:s.structure_label_candidate,uncertain:false,boundary_ok:true})}else if(e.key.toLowerCase()==='u')save(selected,{human_label:'',human_confidence:'',uncertain:true,boundary_ok:true});else if(e.key.toLowerCase()==='b')save(selected,{human_label:'',human_confidence:'',uncertain:false,boundary_ok:false});else if(/^[1-8]$/.test(e.key))save(selected,{human_label:LABELS[+e.key-1],uncertain:false,boundary_ok:true})});
document.querySelector('#split').onchange=renderTracks;document.querySelector('#status').onchange=renderTracks;loadData(false);setInterval(()=>{if(!['TEXTAREA','SELECT'].includes(document.activeElement.tagName))loadData(true)},5000);
</script></body></html>"""
...
class Store:
    ...
    def public_payload(self, access_key: str) -> dict[str, Any]:
        with self.lock:
            scope, read_only = resolve_access(self.payload, access_key)
            ...
            result["access"] = {"scope": scope, "read_only": read_only}
    ...
    def update_annotation(
        ...
        with self.lock:
            scope, read_only = resolve_access(self.payload, access_key)
            if read_only:
                raise PermissionError("the all-results link is read-only")
            assignments = self.payload["annotation_partition"]["assignments"]
            if assignments.get(track_id) != scope:
