...
async function loadData(preserve=true,background=false){if(loading)return;loading=true;const current=preserve&&track?track.track_id:null,visibleTrack=track;try{data=await fetchDataset();track=background&&visibleTrack?visibleTrack:(current?data.tracks.find(t=>t.track_id===current)||null:null);if(!track&&data.tracks.length)track=data.tracks[0];renderTracks();if(!background)renderContent()}catch(e){if(!background||!data){document.querySelector('#content').innerHTML=`<h2 class="warning">无法打开标注数据</h2><p>${esc(e.message)}</p><p class="muted">网络可能暂时不稳定，请刷新重试；页面不会覆盖已经提交的标签。</p>`}else{console.warn('后台同步暂时失败，稍后自动重试',e)}}finally{loading=false}}
