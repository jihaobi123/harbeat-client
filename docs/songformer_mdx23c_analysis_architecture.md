...
这些能力足以开始构建同风格歌曲的候选排序与接歌规则，但不能将《接歌方案》视为已经全部完成。鼓组重合率、分项得分、阈值路由和 pair cache 的独立框架已经具备；MDX23C 五轨到五类落点的自动提取、真实听审校准和正式候选排序接入仍未闭环。固定 6 首 mini set、第二段副歌前出歌、最长等待 8 小节和三种接法的规则也需要按方案逐条做验收测试。
...
| 主要节奏落点 | 三类可比较，五类未闭环 | 现有 16 步主鼓型覆盖 Kick/Snare/Hi-hat；Tom/Cymbal 的 MDX23C 五轨落点还需提取 |
...
1. 在每个 MDX23C 鼓轨上提取 onset，并将 onset 对齐到现有 Beat 和小节位置，补齐 Tom/Cymbal 主鼓型。
2. 为每首歌生成固定长度的鼓型特征，补充每拍命中强度、切分位置和重复稳定性。
