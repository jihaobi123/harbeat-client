...
原表基于同日整理检查；后续已补做只读核对，详见 [后端手册现状审计](../backend-v2/01-current-state.md)，包括实际数据库表与阿里云重复网关服务异常。以下不是可直接执行的安装脚本，也不保证后来无人修改线上 release。
...
| 新手机 App | 外部提供 APK，旧 `mobile/` 不等同其源码 | 主要页面已真机核对，准备终页仍为Mock；源码/联网联调待完成 | 用户选择与控制请求，不负责转发整套音轨 |
...
| `modules/` 保留的 11 个模块 | 第一版实现（含后续维护版本），版本见 modules/REGISTRY.md | 第二版不一定需要，全部待负责人确认；不是必做功能或默认部署项 | 不自动替换正式预处理或线上业务 |
