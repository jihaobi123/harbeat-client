...
| Web 后端和已有数据库模型 | [app](../../app/README.md) | 核心共享引擎已迁出；旧业务路由与数据库仍待 V2 整理 |
...
| 独立模块抽取版本 | [modules 处置表](module-decisions.md) | 13 项已标识历史边界，3 项高重叠实现已比较；未合并删除 |
...
`modules/` 有独立基线和测试；本轮核对的高重叠实现有行为/合同差异，不能按“重复文件名”批量删。详见 [逐项处置](module-decisions.md)。
...
当前完成的是**正式预处理/研究分区，以及共享引擎从业务 app 抽离**，不是完整 V2 后端、客户端重构或历史分支合并。具体部署位置及尚未上线项见 [部署说明](deployment-map.md)。
...
下一批先核对 `app` 业务路由、数据库与发布结果之间的引用，区分可复用业务层和需要新增的 V2 连接层；不把换文件夹当作新接口完成。
随后与前端/RK 负责人确认客户端源码归属，再整理相关代码。远端分支合并、删除和默认分支切换先列明候选及风险，需用户确认；本轮没有执行。
