...
- 整首歌的结构混乱到无法形成可靠段落标签：点击“结构太混乱，不参与”。
...
“结构太混乱，不参与”不会物理删除歌曲或已有标签，而是写入带修订号、操作者、时间和固定原因的
整曲排除标记。被排除歌曲不计入待标注进度，也不会进入训练、开发集完整度检查或锁定测试评估；
页面仍显示该歌曲，并可点击“恢复参与”。排除和恢复操作都会进入审计日志。
...
`--allow-incomplete` 只用于开发集临时实验。只要开发集仍有未审核段落，训练器会明确把测试状态写成
`skipped_incomplete_development`，不会读取或评估锁定测试集；完成全部开发集审核后应去掉该参数重新训练。
