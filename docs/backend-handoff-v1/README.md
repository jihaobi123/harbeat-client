...
> 下文的“正式开工”“已完成”、接口字段和部署命令只描述历史版本，不是第二版实现要求或上线依据。请先读 [参考代码与重构边界](../repository/reference-code.md)。

> **历史业务草案，不是第二版开工依据。** 第二版从 [新后端交接](../v2/backend-owner-handoff.md) 开始，复用现有预处理和数据库，不要求兼容旧手机 API。下文保留历史上下文。
