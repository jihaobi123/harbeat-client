...
离线主页使用现有暖纸白首页结构。页面底部显示 `HUB OFFLINE / RECONNECTING...`，并拦截能量和风格卡片的操作。

如果手环曾收到可信 Snapshot，主页显示最后一次同步的能量、风格和 BPM。如果本次启动还没有收到过 Snapshot，状态字段显示占位值：
...
2. 离线主页显示 `HUB OFFLINE`，首次启动显示占位数据。
