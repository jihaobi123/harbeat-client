...
def _preload_all_in_one_ffmpeg_libraries() -> str | None:
    ...
    lib_dir = (
        ...
        if configured
        else Path(__file__).resolve().parents[2] / ".runtime" / "ffmpeg-shared" / "lib"
    )
    required = [
        "libavutil.59.dylib",
...
def _songformer_work_dir() -> Path:
    ...
    return Path(__file__).resolve().parents[2] / ".runtime" / "songformer-analysis"
...
def _analyze_key_madmom(file_path: str) -> dict:
    ...
    if not external_command:
        adapter = Path(__file__).parents[2] / "scripts" / "madmom_key_cli.py"
        if adapter.is_file():
