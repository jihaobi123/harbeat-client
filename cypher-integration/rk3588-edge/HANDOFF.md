...
| WebSocket | `ws://<RK_LAN_IP>:9000/ws/control?token=<device_token>` |
| 兼容 WS | `ws://<RK_LAN_IP>:9000/ws` |

**不要用** `127.0.0.1` 给手机填；不要用 **9001**（旧版文档端口）。
...
```
ws://<RK_LAN_IP>:9000/ws/control?token=<device_token>
```
...
| `sync_progress` | 下载时 | percent、current_file |
...
HTTP: http://<IP>:9000
WS:   ws://<IP>:9000/ws/control?token=<配对后>
