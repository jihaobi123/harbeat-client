...
**当前已认可的浏览器实时版本：V3.1（3.1.0）**。采用原 V3 选点、完整渐进交接与已试听的动态 EQ。[正式入口](https://8.136.120.255/analysis-lab-static/v31-live-20260924/index.html) · [发布说明与回退入口](docs/V31_RELEASE.md) · [冻结清单](docs/releases/v3.1.0.json)。此前的保护版 V3.1 候选继续归档，和本次正式版区分。
...
## 2026-09-24 乐句与动态 EQ 实验

[实现、边界与验证](docs/PHRASE_ENERGY_CANDIDATE.md) · [独立试听](https://8.136.120.255/analysis-lab-static/v32-candidate-20260924/index.html)。预处理联合声学乐句、尾音与实际小节；A 唱完后再退场；相同选点比较固定与频段功率驱动 EQ。原 V3／V3.1 和已保存偏好保留。当前仅验证原 20 首试听素材，不代表全库已覆盖。

## 2026-09-24 回到 V3 的小幅调整

[实现与验证](docs/V30_SMALL_TUNING.md) · [四项对照与实时试听](https://8.136.120.255/analysis-lab-static/v30-tuning-20260924/index.html)。保留原 V3 完整重叠、线性增益和干湿曲线；边界只做相邻小节的保守核验；独立比较固定与有界动态 EQ。原版本和偏好继续保留。本批原试听位置没有触发边界后移，因此可以先独立判断 EQ。
