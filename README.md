...
裁决或全部模型失败时的兜底。投票前会按最终 BPM + 4/4 拍校验小节周期，自动
淘汰半小节、1.5 倍和双小节层级错误；整数拍相位冲突与前奏漏检会标记人工复核。

段落边界和段落标签只使用原始音频直入的 All-In-One `harmonix-all` 结果；
不再使用能量变化回退，也不再生成固定 8 小节的伪语义 `phrase_map`。能量、
强度等显式特征只附着在 All-In-One 已检测的段落上，不会改变边界和标签。
产品导出的 Bar 1 从 All-In-One 前奏结束后的第一个 downbeat 开始，再按检测
拍号逐拍计数后续小节；前奏内 downbeat 不进入产品小节编号。
...
鼓轨分离完成后，后端会自动检测 Kick、Snare、Hi-hat 候选时间戳，并生成鼓点
...
| `BPM_ENABLE_BEAT_THIS` | `true` | 启用 Beat This BPM 线路 |
