...
def library_song_to_track_context(lib: LibrarySong | None) -> TrackContext:
    ...
    quality = float(getattr(lib, "stem_quality_score", 0.0) or 0.0) if has_stems else 0.0

    activity = getattr(lib, "stem_activity", {}) or {}
    vocal_density = float(activity.get("vocals", 0.5))
    bass_energy = float(activity.get("bass", 0.5))
    ...
    return TrackContext(
        ...
        intro_is_clean=bool(getattr(lib, "intro_is_clean", False)),
        outro_is_clean=bool(getattr(lib, "outro_is_clean", False)),
        has_drum_loop=bool(getattr(lib, "has_drum_loop", False)),
    )
