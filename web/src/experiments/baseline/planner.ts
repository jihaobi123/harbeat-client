...
export function planNext(a:Track,tracks:Track[],position:number,intent:Intent,ready:Set<string>,budget=18,requireReady=true){
 ...
 const exclusions:{trackId:string;track:string;windowId?:string;code:string;reason:string;count:number}[]=[]
 ...
 if(!a.bars.length||a.vocals===null||!Number.isFinite(ae)){exclude(a,'missing_source_evidence','当前曲目缺少拍点、人声或局部能量依据');return {best:null,candidates,exclusions,rejected:[{track:a.title,reason:'当前曲目缺少拍点、人声或局部能量依据'}]}}
 for(const b of tracks){
  ...
  if(intent.kind==='style'&&b.style!==intent.style){exclude(b,'style_mismatch',`模型风格 ${b.style} 不符合目标 ${intent.style}`);continue}
  ...
  for(const w of b.windows){const asset=w.variants[a.id];if(!asset){exclude(b,'missing_variant','没有匹配当前歌曲速度的进入素材',w);continue};const d=asset.duration;const rate=asset.rate
   ...
   if(rate<.8||rate>1.2){exclude(b,'tempo_range',`变速比例 ${rate} 不在 0.8–1.2`,w);continue}
   if(b.duration-w.end<12){exclude(b,'body_too_short','进入窗口后正文不足 12 秒',w);continue}
   if(intent.kind==='up'&&w.energy<ae+.025){exclude(b,'energy_up',`B 能量 ${w.energy} 低于要求 ${ae+.025}`,w);continue}
   if(intent.kind==='down'&&w.energy>ae-.025){exclude(b,'energy_down',`B 能量 ${w.energy} 高于要求 ${ae-.025}`,w);continue}
