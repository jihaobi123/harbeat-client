...
export default function ExperimentLab(){
 ...
 const comparison=useMemo(()=>{try{if(!tracks.length)return null;if(fixed){const f=fixedTrial(tracks,id as 'eq'|'material'|'length',lengthAsset);return {a:f.a,position:f.position,baseline:f.baseline,variant:f.variant,searches:null}}
 const a=tracks.find(t=>t.id===aId);if(!a||!Number.isFinite(position)||position<0||position>=a.duration)throw new Error('请选择有效的歌曲和原曲触发位置');const b=planTrial(a,tracks,position,'baseline',target||undefined),v=planTrial(a,tracks,position,id as RankingId,target||undefined);return {a,position,baseline:b.best,variant:v.best,searches:{baseline:b,variant:v}}
 }catch(e){return {error:(e as Error).message}}},[tracks,id,aId,target,position,lengthAsset,fixed])
 const key=JSON.stringify([catalogId,id,aId,target,position]),currentHeard=heard.filter(x=>x.startsWith(key+':'))
 ...
 async function play(arm:'baseline'|'variant'){
  ...
  try{if(!player.current){const live=new LiveTransport(tracks,()=>refresh(x=>x+1),base);player.current=live;await live.initialize();if(token!==generation.current)return}
   ...
   live.log('comparison_analysis',{comparisonId:run.current,catalogSha256:catalogId,experimentId:id,arm,config:cfg,referenceCommit:'279b65d',comparison})
   await live.playComparison(plan,comparison.position,{experimentId:id,arm,comparisonId:run.current,midDb:id==='eq'&&arm==='variant'?-8:-5,horizonSec:arm==='baseline'?18:cfg.horizonSec,config:arm==='baseline'?experiments.baseline:cfg,catalogSha256:catalogId})
 ...
 function rate(value:string){if(!p||!bothCompleted)return;p.log('listening_comparison',{experimentId:id,comparisonKey:key,preference:value,note,config:cfg,catalogSha256:catalogId});setRating('已记录：'+value)}
 ...
 {fixed?<p>固定曲对：Big Girls → Cold；触发位置 15 秒，A 在 27.040 秒退出。此组使用一个可执行的固定候选，专门比较声音处理／素材，不用来证明自动选点更好。</p>:<div className="trial-controls"><label>当前歌曲<select disabled={locked} value={aId} onChange={e=>{setAId(e.target.value);setTarget('')}}>{tracks.map(t=><option key={t.id} value={t.id}>{t.title}</option>)}</select></label><label>原曲触发位置（秒）<input type="number" min="0" step="1" disabled={locked} value={position} onChange={e=>setPosition(e.target.valueAsNumber)}/></label><label>下一首范围<select disabled={locked} value={target} onChange={e=>setTarget(e.target.value)}><option value="">在全部候选中选择</option>{tracks.filter(t=>t.id!==aId).map(t=><option key={t.id} value={t.id}>{t.title}</option>)}</select></label></div>}
 {audit&&!fixed&&<details><summary>六首真实素材的规则对照结果（非听感评分）</summary><pre>{JSON.stringify(audit.summary?.find((x:any)=>x.trial===id),null,2)}</pre><div className="trial-cases">{audit.changedCases?.filter((c:any)=>c.trial===id).slice(0,10).map((c:any,i:number)=><button key={i} disabled={locked} onClick={()=>{setAId(tracks.find(t=>t.title===c.a)!.id);setPosition(c.position);setTarget('')}}>{c.a} · {c.position} 秒</button>)}</div></details>}
 ...
 <h3>你的试听结论</h3><textarea value={note} onChange={e=>setNote(e.target.value)} placeholder="例如：人声更清楚，但等待稍长；或副歌被提前切断"/><div className="trial-play">{['参照更好','实验更好','差不多','都有问题'].map(v=><button disabled={!bothCompleted} key={v} onClick={()=>rate(v)}>{v}</button>)}</div><p>{rating||'两段都播放到结束后，可以记录偏好；统计不会代替你的选择。'}</p></>}
