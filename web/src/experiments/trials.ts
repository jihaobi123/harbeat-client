...
export function fixedTrial(tracks:Track[],id:'eq'|'material'|'length',lengthAsset?:LengthAsset){
 const a=tracks.find(t=>t.title==='Big Girls'),b=tracks.find(t=>t.title==='Cold');if(!a||!b)throw new Error('固定对照需要 Big Girls 和 Cold')
 const reference=planTrial(a,tracks,15,'baseline',b.id).candidates.find(p=>p.window.id==='w0-4'&&Math.abs(p.end-27.04)<.001)
 ...
 if(id==='material'){
  const w=b.windows.find(w=>w.id==='w16-4'),asset=w?.variants[a.id]
 ...
 if(id==='length'){
  ...
  if(variant.start<15+.25||Math.min(...a.bars.map(v=>Math.abs(v-variant.start)))>.065)throw new Error('缩短窗口无法在同一请求时刻按拍执行')
 }
 for(const [label,p] of [['baseline',baseline],['variant',variant]] as const){
  ...
  p.decision=explainPlan(a,b,p,15,{kind:'next',targetId:b.id},18,true);p.decision.policyVersion=`quality-fixed-1:${id}:${label}`;p.decision.pointReasons=['固定对照使用基线可执行候选，不是普通下一首的最高分结果。',label==='baseline'?'固定曲对参照：4 小节窗口，原 EQ。':experiments[id].change]
 ...
 return {a,b,position:15,baseline,variant}
}
