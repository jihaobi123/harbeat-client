...
export class LiveTransport{
 ...
 log(kind:string,data:Log={}){this.logs.push(JSON.parse(JSON.stringify({kind,sessionId:this.sessionId,sequence:this.logs.length+1,wallTime:new Date().toISOString(),contextSec:this.ctx.currentTime,...data})))}
 ...
 private auditSearch(requestId:string,phase:string,result:ReturnType<typeof planNext>,position:number,budget:number){
  const ranked=result.candidates.map((p,rank)=>({rank:rank+1,id:p.id,to:p.to,windowId:p.window.id,start:p.start,end:p.end,score:p.score,prepared:p.prepared,scoreComponents:p.decision?.score.components,reason:p.reason}))
 ...
 private tick(){if(this.dead)return;this.sync();if(this.active&&this.ctx.state==='running'&&!this.paused&&!this.pending&&!this.busy){const left=this.active.track.duration-this.position;if(left<20&&!this.autoTried){this.autoTried=true;void this.request({kind:'next'},18,{origin:'source_end_auto'})}if(left<=.01){this.stop();this.status='本段试听已播完';this.log('source_end')}}this.update()}
