...
class V3Clock extends AudioWorkletProcessor{
 ...
 process(inputs,outputs){
  ...
  for(let j=0;j<n;j++){
   ...
   this.minimumGain=Math.min(this.minimumGain,this.gain)
  ...
  for(let i=this.events.length-1;i>=0;i--){const e=this.events[i];if(e.frame<currentFrame+n){this.port.postMessage({...e,observedFrame:currentFrame+n,quantumStart:currentFrame,quantumFrames:n,late:Math.max(0,currentFrame-e.frame),limiterDelayFrames:this.delay,peak:this.maximum,limiterGainAtObservation:this.gain,sessionMinimumLimiterGain:this.minimumGain});this.events.splice(i,1)}}
  return true
 }
}
