    ...
    for lib_song_id in library_song_ids:
        ...
        if not song:
            song = Song(
                ...
                audio_url=lib_song.source_path or None,
            )
            db.add(song)
            db.flush()
        elif (not song.audio_url or not os.path.isfile(song.audio_url)) and lib_song.source_path:
            song.audio_url = lib_song.source_path
        ...
        elif song.tags.bpm is None and lib_song.bpm:
            song.tags.bpm = int(lib_song.bpm)
...
def generate_style_mix_playlist(db: Session, payload: StyleMixRequest, user_id: int) -> StyleMixResult:
    """
    缁冭垶浼氳瘽闀挎瓕鍗曠敓鎴愶細
    1. 浠庢湇鍔″櫒鏇插簱鎸?style/bpm/energy 绛涙瓕
    2. 绱鍒扮洰鏍囨椂闀?
    3. 瀵规瘡棣栨瓕搴旂敤鍗曟洸椋庢牸澶勭悊锛屽苟杩斿洖妯″瀷閫夋嫨淇℃伅
    """
    songs: list[Song] = []
...
def generate_dj_offline_mix(db: Session, payload: DjOfflineMixRequest) -> DjOfflineMixResult:
    ...
    for i, candidate in enumerate(render_track_candidates):
        ...
        entry_time = 0.0
        if i > 0:
            prev_song_id = int(render_track_candidates[i - 1]["song_id"])
