...
class TransitionPlan:
    ...
    vocal_cut_ratio: float | None = None
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "vocal_cut_ratio": self.vocal_cut_ratio,
            # Jetson/RK3588-compatible transition timing fields
            "from_beat_interval_sec": round(60.0 / max(self.bpm_from, 1.0), 4),
            "to_beat_interval_sec": round(60.0 / max(self.bpm_to, 1.0), 4),
            "from_at_sec": 0.0,
            "from_out_sec": round(self.duration_bars * 4.0 * 60.0 / max(self.bpm_from, 1.0), 3),
            "to_at_sec": 0.0,
            "to_in_sec": 0.0,
            "fade_sec": 0.0,
            "crossfade_sec": round(self.duration_bars * 4.0 * 60.0 / max(self.bpm_from, 1.0), 3),
    ...
    def from_dict(cls, d: dict[str, Any]) -> "TransitionPlan":
        return cls(
            ...
            vocal_cut_ratio=d.get("vocal_cut_ratio"),
        )
...
def _vocal_handoff_curves(duration_bars: int, bpm_from: float, bpm_to: float,
                             vocal_cut_ratio: float | None = None) -> list[AutomationCurve]:
    """Avoid dual vocal clash: A vocals echo out at phrase boundary, B vocals enter late.

    When vocal_cut_ratio is provided (0.0–1.0), exit_point snaps to the nearest
    bar boundary within the transition window. Default 0.35 → ~2.8 bars.
    """
    if vocal_cut_ratio is not None and duration_bars > 0:
        # Snap ratio to nearest bar boundary: round(ratio * bars) / bars
        snapped = round(vocal_cut_ratio * duration_bars) / duration_bars
        exit_point = max(0.125, min(0.875, snapped))
    else:
        exit_point = 0.35
    entry_point = exit_point + 0.25
    return [
        AutomationCurve(CurveTarget.A_vocals, CurveParam.gain,
...
def generate_plan(
    ...
    stem_ok = has_stems and from_ctx.stem_quality_score >= 0.4 and to_ctx.stem_quality_score >= 0.4
    if stem_ok and stem_required:
        if preset == TransitionPreset.vocal_handoff and vocal_cut_ratio is not None:
            curves = stem_fn(duration_bars, from_ctx.bpm or 120.0, to_ctx.bpm or 120.0,
                            vocal_cut_ratio=vocal_cut_ratio)
        else:
            curves = stem_fn(duration_bars, from_ctx.bpm or 120.0, to_ctx.bpm or 120.0)
...
def select_best_preset(
    ...
    1. No stems or low stem quality → only non-stem presets
    2. Key incompatible (distance >= 3) → no acapella_overlay, short transitions
    3. Dual vocal high density → vocal_handoff or echo_freeze
    4. Dual bass high energy → bass_swap mandatory
    5. Weak B intro + stable A drums → drum_bridge
    6. Clean breakdown/drop → breakdown_drop
    7. Low confidence → short echo/cut, never long overlap
    ...
    if not stem_ok:
        ...
        if key_ok and scores.bpm_distance < 0.3:
            candidates.append((0.7, TransitionPreset.bass_swap))  # becomes filter fallback
...
def build_automix_transition(
    ...
    if tempo_strategy is None:
        if scores.bpm_distance < 0.02:
            tempo_strategy = TempoStrategy.none
        elif scores.bpm_distance < 0.06:
            tempo_strategy = TempoStrategy.sync_to_from
        elif scores.bpm_distance < 0.08:
