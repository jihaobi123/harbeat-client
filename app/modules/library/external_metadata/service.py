...
def _local_source_labels(song) -> list[str]:
    ...
    if isinstance(gp, dict):
        ...
        for key in ("spotify_genres_raw", "discogs_labels_raw"):
            for item in gp.get(key) or []:
    ...
    tag = getattr(catalog, "tags", None) if catalog is not None else None
    if tag is not None:
        for key in ("style", "groove_tag", "energy", "vocal_type", "era_tag", "difficulty_fit"):
...
async def _fetch_external_sources(
    *,
    ...
    if fetcher is None:
        return await fetch_all_external_metadata(
            title=title,
            artist=artist,
            timeout_sec=timeout,
        )
    maybe = fetcher(title=title, artist=artist, timeout_sec=timeout)
    return await maybe if hasattr(maybe, "__await__") else maybe


def _run_coroutine_blocking(factory: Callable[[], Coroutine[object, object, T]]) -> T:
    """Run async HTTP fetching from sync callers, even inside an existing loop."""
    try:
        asyncio.get_running_loop()
    except RuntimeError:
        return asyncio.run(factory())

    q: Queue[tuple[bool, T | BaseException]] = Queue(maxsize=1)

    def runner() -> None:
        try:
            q.put((True, asyncio.run(factory())))
        except BaseException as exc:
            q.put((False, exc))

    thread = Thread(target=runner, name="style-enrich-fetch", daemon=True)
    thread.start()
    thread.join()
    ok, value = q.get()
    if ok:
...
def _disabled_sources() -> dict[str, ExternalSourceResult]:
    return {
...
def _error_sources(message: str = "fetch failed") -> dict[str, ExternalSourceResult]:
    return {
        "musicbrainz": ExternalSourceResult.error_result("musicbrainz", message),
        "lastfm": ExternalSourceResult.error_result("lastfm", message),
        "discogs": ExternalSourceResult.error_result("discogs", message),
    }
...
def _apply_external_enrichment(
    ...
    *,
    ...
    for style in STYLE_PROFILES:
        ...
        if local_score > 0:
            reasons.append(f"local fingerprint {local_version} score {local_score:.2f}")
...
def run_enrich_song_external_metadata(
    ...
    *,
    ...
    if not settings.enable_external_style_enrichment:
        sources = _disabled_sources()
    else:
        try:
            sources = _run_coroutine_blocking(
                lambda: _fetch_external_sources(
                    title=str(getattr(song, "title", "") or ""),
                    artist=str(getattr(song, "artist", "") or ""),
                    timeout=timeout,
                    fetcher=fetcher,
            ...
            logger.exception("[style-enrich] external fetch failed for song %s", getattr(song, "id", ""))
            sources = _error_sources()

    return _apply_external_enrichment(db, song, sources, commit=commit)
