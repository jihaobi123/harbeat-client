...
**第二版后续重构，预计不直接使用。** 旧接口、数据库模型和后台任务保留以便理解存量系统，不作为新后端的默认实现。下文“当前职责”描述旧代码，不代表第二版必须保留。正式分析实现已迁出，旧导入转发暂留以保护现有调用。见 [参考代码与重构边界](../docs/repository/reference-code.md)。

已确认的第二版方向：按用户提供的新 APK 源码/真实页面和当前预处理合同重构，不要求兼容旧手机 API。旧 Python 导入转发只是现有后台的过渡保护，不是新 App 的接口要求。新 APK 源码待用户上传，本轮不据旧 mobile 推测接口。
